'use client'

import { useState } from 'react'
import Link from 'next/link'
import Image from 'next/image'
import NSFASModal from './NSFASModal'

export default function Footer() {
  const [isModalOpen, setIsModalOpen] = useState(false)

  return (
    <>
      <NSFASModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
      <footer className="bg-navy text-white mt-auto">
        <div className="container mx-auto px-4 py-12">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {/* About */}
            <div>
              <div className="flex items-center space-x-3 mb-4">
                <div className="relative w-12 h-12">
                  <Image
                    src="/assets/logo/Concordia House.png"
                    alt="Concordia House Logo"
                    fill
                    className="object-contain"
                  />
                </div>
                <h3 className="text-xl font-bold">Concordia House</h3>
              </div>
              <p className="text-gray-300 text-sm leading-relaxed">
                Exclusive student residence in Komani. Where fun and achievement
                go hand in hand.
              </p>
            </div>

            {/* Quick Links */}
            <div>
              <h3 className="text-xl font-bold mb-4">Quick Links</h3>
              <ul className="space-y-2 text-sm">
                <li>
                  <button
                    onClick={() => setIsModalOpen(true)}
                    className="text-gray-300 hover:text-terracotta transition-colors text-left"
                  >
                    Apply / Waitlist
                  </button>
                </li>
                <li>
                  <Link
                    href="/rules"
                    className="text-gray-300 hover:text-terracotta transition-colors"
                  >
                    Rules & Regulations
                  </Link>
                </li>
                <li>
                  <Link
                    href="/incident"
                    className="text-gray-300 hover:text-terracotta transition-colors"
                  >
                    Report Incident
                  </Link>
                </li>
                <li>
                  <Link
                    href="/suggestions"
                    className="text-gray-300 hover:text-terracotta transition-colors"
                  >
                    Suggestion Box
                  </Link>
                </li>
              </ul>
            </div>

            {/* Contact */}
            <div>
              <h3 className="text-xl font-bold mb-4">Contact Us</h3>
              <div className="space-y-3 text-sm">
                <p className="text-gray-300">
                  <strong>Location:</strong> Komani, South Africa
                </p>
                <p className="text-gray-300">
                  <strong>Distance:</strong> 1.5km from WSU Campus
                </p>
                <a
                  href="https://wa.me/27839940008"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center space-x-2 text-terracotta hover:text-terracotta-light transition-colors"
                >
                  <svg
                    className="w-5 h-5"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                  </svg>
                  <span>Chat on WhatsApp</span>
                </a>
              </div>
            </div>
          </div>

          <div className="border-t border-gray-700 mt-8 pt-8 text-center text-sm text-gray-400">
            <p>&copy; {new Date().getFullYear()} Concordia House. All rights reserved.</p>
            <p className="mt-2">NSFAS Grade A Accredited | Exclusive Residence</p>
          </div>
        </div>
      </footer>
    </>
  )
}

